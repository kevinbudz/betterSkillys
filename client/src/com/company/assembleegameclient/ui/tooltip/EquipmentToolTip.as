...
public class EquipmentToolTip extends ToolTip
   {
      ...
      public function EquipmentToolTip(objectType:int, player:Player, invType:int, inventoryOwnerType:String, inventorySlotID:uint = 1.0, isMarketItem:Boolean = false, itemData:Object = null)
      {
         ...
         this.itemData = ItemData.FromXML(objectType);

         var equips:Vector.<ItemData> = new Vector.<ItemData>;
         for (var i:int = 0; i < 4; i++)
         {
            equips[i] = ItemData.FromXML(player.equipment_[i]);
         }
         if (player) {
            this.player_ = player;
            var equipId:int = GetEquipIndex(itemData.SlotType, equips);
            if (equipId != -1)
               this.equipData = ItemData.FromXML(player.equipment_[equipId]);
         }
         trace (this.itemData);
         trace (this.equipData);
